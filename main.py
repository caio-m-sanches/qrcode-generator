...
from utils import setup_qrcode
...
with c1:    
    ...
    text = st.text_input("QR Code suporta até no máximo 4 mil caracteres.", key = 'content')

    c11, c12 = st.columns(2)

    with c11:
        color = st.color_picker("Escolha uma cor", "#000000", key='color_picker')
    with c12:

        st.selectbox(
            "Selecine o formato", ['png', 'svg'], 
            key = 'format', 
        )
    ...
    with c13:
        ...
        if format == 'svg':
            ...
            st.selectbox(
                "Selecine a resolução", ['256 x 256', '512 x 512', '1024 x 1024'],
